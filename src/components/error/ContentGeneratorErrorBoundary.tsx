...
export class ContentGeneratorErrorBoundary extends Component<Props, State> {
  ...
  static getDerivedStateFromError(error: Error): State {
    console.error('🚨 ContentGenerator Error Boundary caught an error:', error);
    return { hasError: true, error };
  }

  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error('🚨 ContentGenerator Error Boundary details:', error, errorInfo);
    this.setState({
      error,
      errorInfo
    });
  ...
  handleRetry = () => {
    console.log('🔄 ContentGenerator Error Boundary: Retrying...');
    this.setState({ hasError: false, error: undefined, errorInfo: undefined });
  };

  handleGoHome = () => {
    console.log('🏠 ContentGenerator Error Boundary: Going to dashboard...');
          ...
          <Card className="max-w-2xl mx-auto mt-8 border-red-200 bg-red-50">
            ...
            <CardContent className="space-y-4">
              <div className="text-red-600">
                <p className="font-medium mb-2">The content generator encountered an error.</p>
                <p className="text-sm">
